...
SBSBigBite::SBSBigBite( const char* name, const char* description,
			UInt_t nsectors, Bool_t cer, Bool_t sci, Bool_t cal ) :
  THaSpectrometer( name, description )
{
  // Constructor. Defines standard detectors
  static const char* const here = "SBSBigBite";
  ...
  */
  if( nsectors == 0 ) {
    Warning( Here(here), "Creating SBS with no GEMs" );
  }else{
  ...
  if(cer){
    SBSGRINCH* theGrinch = new SBSGRINCH( "grinch" , "GRINCH detector");
    // When you want to add a new detector (of *any* kind) to the spectrometer,
    // you have to add it to the spectrometer detector list THaSpectrometer::fDetectors
    // zith the function AddDetector
    Int_t ret = AddDetector( theGrinch );
    /*
    if( ret != 0 ) {
      stringstream s;
      s << "Error adding GEM tracker \"" << sn.str() << "\" "
...
//_____________________________________________________________________________
THaAnalysisObject::EStatus SBSBigBite::Init( const TDatime& run_time )
{
...
Int_t SBSBigBite::ReadRunDatabase( const TDatime& date )
{
  ...
  // This is a bit of a kludge. SBSBigBite actually shouldn't inherit from
