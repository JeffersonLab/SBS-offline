...
SBSTimingHodoscope::SBSTimingHodoscope( ) : THaSubDetector("plane","scintplane",0),
...
//_____________________________________________________________________________
THaAnalysisObject::EStatus SBSTimingHodoscope::Init( const TDatime& date )
{
...
Int_t SBSTimingHodoscope::Decode( const THaEvData& evdata )
{
...
#if DEBUG_LEVEL>=4//massive info
    clog <<"-- SBSTimingHodoscope (name: "<<GetName()<<") Decode THaEvData of Run#"<<evdata.GetRunNum()<<" --"<<endl;
    clog<<"EvType="<<evdata.GetEvType()<<"; EvLength="<<evdata.GetEvLength()<<"; EvNum="<<evdata.GetEvNum()<<endl;
#endif//#if DEBUG_LEVEL>=4

#if DEBUG_LEVEL>=5//super massive info
    clog<<">>>>Extensive search in SBSTimingHodoscope::Decode";
    {
        int crate,slot,chan;
    ...
    if (i!=GetNRefCh()) {
        clog<<"SBSTimingHodoscope::Decode : Mismatch between fNRefCh and hits on RefLines"<<endl;
