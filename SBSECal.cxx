...
//_____________________________________________________________________________
Int_t SBSECal::FineProcess(TClonesArray& tracks)
{
...
void SBSECal::AddCluster(SBSECalCluster* clus) {
    fClusters[fNclust++]=clus;
}

void SBSECal::AddCluster(SBSECalCluster& clus) {

    fClusters[fNclust] = new SBSECalCluster(clus.GetNMaxBlocks());
...
void SBSECal::RemoveCluster(int i) {
    fNclust--;
    for (int j=i;j<fNclust;j++) fClusters[j]=fClusters[j+1];
}

Int_t SBSECal::BlockColRowToNumber( Int_t col, Int_t row )
{
    return col*fNrows + row;
}

void SBSECal::LoadMCHitAt( Double_t x, Double_t y, Double_t E )
{  
    ClearEvent();
    fNclust = 0;
    fClusters[fNclust] = new SBSECalCluster(0);
