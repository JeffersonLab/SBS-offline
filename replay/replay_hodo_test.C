...
void replay_hodo_test(Int_t runnum = 931, Int_t lastEvent = -1){
  ...
  SBSEArm* bigbite = new SBSEArm("bb", "BigBite spectrometer" );
  bigbite->AddDetector( new SBSTimingHodoscope("hodo", "BigBite hodoscope") );
  ...
  THaRun* run = new THaRun(TString::Format("data/hodotest_%d.dat",runnum) );
  ...
  analyzer->SetOdefFile("output_hodo_test.odef");
